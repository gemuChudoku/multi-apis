...
app.post("/refunds", async (req, res) => {
  const { sale_id, product_name, client_name, seller, quantity, unit_price, total_amount, reason, status } = req.body ?? {};
  
  if (!sale_id || !product_name || !client_name || !seller || !quantity || !unit_price || !total_amount || !reason) {
    return res.status(400).json({ error: "sale_id, product_name, client_name, seller, quantity, unit_price, total_amount y reason son requeridos" });
  }

  try {
    const r = await pool.query(
      `INSERT INTO refunds_schema.refunds (sale_id, product_name, client_name, seller, refund_date, quantity, unit_price, total_amount, reason, status) 
       VALUES($1, $2, $3, $4, CURRENT_DATE, $5, $6, $7, $8, $9) 
       RETURNING *`,
      [sale_id, product_name, client_name, seller, quantity, unit_price, total_amount, reason, status || 'pending']
    );
...
app.put("/refunds/:id", async (req, res) => {
  ...
  if (!sale_id || !product_name || !client_name || !seller || !quantity || !unit_price || !total_amount || !reason || !status) {
  ...
  try {
    const r = await pool.query(
      `UPDATE refunds_schema.refunds 
       SET sale_id = $1, product_name = $2, client_name = $3, seller = $4, quantity = $5, 
           unit_price = $6, total_amount = $7, reason = $8, status = $9 
       WHERE id = $10 
       RETURNING *`,
      [sale_id, product_name, client_name, seller, quantity, unit_price, total_amount, reason, status, id]
    );
